...
use asset_io::test_utils::{fixture_path, is_embedded, list_fixtures};
    ...
    for (filename, result) in &results {
        match result {
            Ok(info) => {
                ...
                println!(
                    "  XMP Mod:  {}",
                    if info.xmp_modify_works { "✓" } else { "✗" }
                );
                println!(
                    "  JUMBF Rm: {}",
                    if info.jumbf_remove_works {
                        "✓"
                    } else {
                        "✗"
                    }
...
fn test_fixture(path: &str) -> asset_io::Result<FixtureInfo> {
    ...
    std::process::Command::new("identify")
        ...
        .output()
        .map_err(|e| {
            asset_io::Error::Io(std::io::Error::new(
