...
fn main() {
    ...
    #[cfg(feature = "png")]
    {
        use asset_io::{PngHandler, FormatHandler};
        
        let png_data = create_png_with_exif();
        println!("Created minimal PNG with eXIf chunk: {} bytes", png_data.len());
        ...
        let exif_segment = structure.segments().iter().find(|s| {
            matches!(s, asset_io::Segment::Exif { .. })
        });
        ...
        let output_structure = handler.parse(&mut output_cursor).expect("Failed to parse written PNG");
        
        let has_exif_output = output_structure.segments().iter().any(|s| {
            matches!(s, asset_io::Segment::Exif { .. })
        });
