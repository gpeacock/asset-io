...
fn main() -> asset_io::Result<()> {
    #[cfg(feature = "test-utils")]
    {
        ...
        println!("\nKey Features:");
        println!("  ✓ Constant memory usage (only {:.1}KB buffers)", DEFAULT_CHUNK_SIZE as f64 / 1024.0);
...
fn compute_data_hash(path: &str) -> asset_io::Result<Vec<u8>> {
    ...
    for (i, range) in ranges.iter().enumerate() {
        println!("     Range {}: offset={}, size={} bytes", 
                 i + 1, range.offset, range.size);
    }
...
fn compute_box_hash(path: &str) -> asset_io::Result<Vec<u8>> {
    ...
    let segments: Vec<_> = asset.structure().segments_excluding(&exclusions)
        .into_iter()
    ...
    for (index, path_str, loc) in segments {
        println!("     Segment {}: {} ({} bytes)", 
                 index, path_str, loc.size);
...
fn compute_chunked_hash(path: &str, chunk_size: usize) -> asset_io::Result<Vec<u8>> {
    ...
    println!("   Processed {} bytes in {}KB chunks", 
             total_bytes, chunk_size / 1024);
