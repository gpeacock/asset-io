...
fn test_mmap_bounds_checking_with_overflow() {
    ...
    assert!(structure.get_mmap_slice(bad_range).is_none(), 
    ...
    assert!(structure.get_mmap_slice(overflow_range).is_none(),
            "Overflow should return None");
    
    // Test 3: Valid range works
    let good_range = ByteRange {
        offset: 0,
        size: 2,
    };
    assert!(structure.get_mmap_slice(good_range).is_some(),
